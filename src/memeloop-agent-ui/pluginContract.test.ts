...
describe('MemeLoop TiddlyWiki plugin integration contract', () => {
  ...
  it('composes shared major Agent features through a replaceable host boundary', () => {
    const source = read('components.tsx');
    expect(source).toContain('createFileAttachmentSource');
    expect(source).toContain('createWikiAgentConversationClient');
    expect(source).toContain('createWikiAgentInstanceClient');
    ...
    expect(source).toContain('mapFile: createFileAttachmentSource');
